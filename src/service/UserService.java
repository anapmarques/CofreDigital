...
public class UserService {
    ...
    public void createAdminInteractive() throws Exception {
        ...
        byte[] totpEnc = crypto.EnvelopeUtil.encrypt(totp);
    ...
    public void registerInteractive() throws Exception {
        ...
        boolean valid = false;
        try {
            byte[] sig = crypto.SignatureUtil.sign(priv, challenge);
            valid = crypto.SignatureUtil.verify(cert, challenge, sig);
        } catch (Exception ex) {
            valid = false;
        }
        if (!valid) {
        ...
        byte[] totp = new byte[20]; rnd.nextBytes(totp);
        byte[] totpEnc = crypto.EnvelopeUtil.encrypt(totp);
        byte[] privEnc = crypto.EnvelopeUtil.encrypt(priv);
